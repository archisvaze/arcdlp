...
window.api.onLog((msg) => {
    ...
    if (
        ...
        msg.startsWith("Error") || msg.includes("failed") ||
        msg.includes("Failed:")
    ) type = "error";
...
async function doFetch() {
    ...
    try {
        ...
        if (isAuthError(msg)) {
            ...
            showError(
                "yt-dlp binary not found. Run `npm install` to download it.",
            );
        } else if (
            msg.includes("Unsupported URL") || msg.includes("No video formats")
        ) {
            showError("This URL is not supported or the video is unavailable.");
        } else if (
            msg.includes("HTTP Error 403") || msg.includes("HTTP Error 429")
        ) {
...
async function fetchPlaylist(url) {
    ...
    $plItemCount.textContent = `${playlistItems.length} item${
...
function showCard() {
    ...
    if (videoInfo.extractor_key) {
        meta.push(
            `<span class="meta-item"><span class="meta-badge">${
                escapeHtml(videoInfo.extractor_key)
            }</span></span>`,
        );
    }
    if (videoInfo.live_status === "is_live") {
        meta.push(
            `<span class="meta-item"><span class="meta-badge live">LIVE</span></span>`,
        );
    }
    if (videoInfo.webpage_url_domain) {
        meta.push(
            `<span class="meta-item">${
                escapeHtml(videoInfo.webpage_url_domain)
            }</span>`,
        );
    }
    ...
    $vidFmts.innerHTML = videoPresets.map((p) => formatOptionHTML(p, "🎬"))
        .join("");
...
function appendPlaylistItem(item) {
    ...
    el.innerHTML = `
        <label class="pl-item-check">
            <input type="checkbox" ${checked} onchange="togglePlaylistItem('${
        escapeHtml(item.id)
    }', this.checked)" />
...
function playlistSelectAll() {
    ...
    $plItems.querySelectorAll('input[type="checkbox"]').forEach((
        cb,
    ) => (cb.checked = true));
    updatePlDownloadBtn();
}
...
function updatePlDownloadBtn() {
    ...
    $plDownloadBtn.textContent = count > 0
        ? `Add ${count} item${count !== 1 ? "s" : ""} to Queue`
...
async function doPlaylistDownload() {
    ...
    const selected = playlistItems.filter((item) =>
        playlistSelected.has(item.id)
    );
...
function renderQueue() {
    ...
    $qRetryFailedBtn.style.display = counts.failed > 0
        ? "inline-block"
        : "none";
    $qClearDoneBtn.style.display =
        counts.completed > 0 || (counts.failed > 0 && !isActive)
            ? "inline-block"
            : "none";
...
function queueItemHTML(item) {
    ...
    if (item.state === "pending") {
        ...
        actionsHTML =
            `<button class="q-item-remove" data-id="${item.id}" title="Remove">×</button>`;
        ...
        statusHTML = `<span class="q-item-status downloading">${pct}${
            detail ? " · " + detail : ""
        }</span>`;
        progressHTML =
            `<div class="q-item-progress-track"><div class="q-item-progress-fill" style="width:${pctNum}%"></div></div>`;
        actionsHTML =
            `<button class="q-item-cancel" title="Skip this item">⏭</button>`;
    } else if (item.state === "completed") {
        statusHTML = '<span class="q-item-status completed">Complete ✓</span>';
        actionsHTML =
            `<button class="q-item-open-folder" title="Open folder" onclick="doOpenFolder()">📂</button><button class="q-item-remove" data-id="${item.id}" title="Remove">×</button>`;
    } else if (item.state === "failed") {
        const errText = escapeHtml(item.error || "Failed");
        statusHTML =
            `<span class="q-item-status failed" title="${errText}">${errText}</span>`;
        actionsHTML =
            `<button class="q-item-retry" data-id="${item.id}" title="Retry">↻</button><button class="q-item-remove" data-id="${item.id}" title="Remove">×</button>`;
    }
...
function showAuthError() {
    const text = isSignedIn
        ? "This video requires sign-in. Your session may have expired. "
        : "This video requires sign-in. ";
    const btnLabel = isSignedIn ? "Sign in again" : "Sign in to YouTube";
    $error.innerHTML = escapeHtml(text) +
        `<button class="auth-error-btn" onclick="doLogin()">${btnLabel}</button>`;
...
function updateAuthUI() {
    ...
    if (isSignedIn) {
        el.innerHTML =
            '<span class="auth-signed-in">Signed in to YouTube</span>';
...
function renderHistory() {
    ...
    $historyCountLabel.textContent = `${list.length} item${
        list.length !== 1 ? "s" : ""
    }`;
    ...
    $historyList.innerHTML = list
        .map((entry, idx) => {
            ...
            <div class="history-item" data-idx="${idx}">
                ...
                subParts.join(' <span class="meta-sep">·</span> ')
            }</div>
                </div>
                <div class="history-actions">
                    <button class="history-remove" data-id="${
                escapeHtml(id)
            }" data-ext="${escapeHtml(ext)}" title="Remove">×</button>
                </div>
            </div>`;
        })
        .join("");
...
function loadFromHistory(idx) {
    ...
    if (!entry.presets || entry.presets.length === 0) {
        ...
        switchTab("download");
        addLog(
            "URL loaded from history - click Fetch to get video info",
            "highlight",
        );
        return;
    }
...
async function loadSettings() {
    ...
    try {
        ...
        ytdlpEl.textContent = deps.ytdlp.found
            ? deps.ytdlp.path
        ...
        ffEl.textContent = deps.ffmpeg.found
            ? deps.ffmpeg.path
            : "Not found - run npm install";
...
async function init() {
    ...
    try {
        const info = await window.api.getAppInfo();
        addLog(
            `v${info.version} · ${info.platform}/${info.arch} · dev:${info.devMode}`,
        );
    } catch {}
    ...
    try {
        const deps = await window.api.checkDeps();
        addLog(
            `yt-dlp: ${deps.ytdlp.found ? "✓" : "✗ NOT FOUND"}`,
            deps.ytdlp.found ? "success" : "error",
        );
        addLog(
            `ffmpeg: ${deps.ffmpeg.found ? "✓" : "✗ NOT FOUND"}`,
            deps.ffmpeg.found ? "success" : "error",
        );
