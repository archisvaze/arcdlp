...
let queueData = { items: [], counts: { total: 0, pending: 0, downloading: 0, completed: 0, failed: 0 }, isActive: false };
let hasAutoSwitchedToQueue = false; // only auto-switch once per session
...
window.api.onLog((msg) => {
    ...
    else if (msg.startsWith('[') || msg.startsWith('Downloading ')) type = 'highlight';
    addLog(msg, type);
});
...
function renderQueue() {
    ...
    // Status text
    const statusParts = [];
    if (counts.downloading > 0) statusParts.push(`${counts.downloading} downloading`);
...
function renderHistory() {
    ...
    $historyList.innerHTML = list
        .map((entry, idx) => {
            ...
            const subParts = [];
            if (source) subParts.push(`<span class="source-badge">${escapeHtml(source)}</span>`);
            ...
            <div class="history-item" data-idx="${idx}">
                ${
                    thumb
                        ? `<img class="history-thumb" src="${escapeHtml(thumb)}" alt="" onerror="this.style.display='none';this.nextElementSibling.style.display='flex';" /><div class="history-thumb-error">🎬</div>`
